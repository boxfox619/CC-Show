...
class AssetStore extends React.Component {
    ...
    render() {
        ...
        let renderAssetItems = (assets) => {
            return assets.map((asset) => {
                return (<AssetItem key={'assetitem' + asset.id} deleteAsset={() => this.deleteAsset(asset.id)}
                                   onClick={() => this.previewAsset(asset.id)} useAsset={() => this.useAsset(asset.id)}
                                   id={asset.id} title={asset.title} subTitle={asset.user} star={asset.star}
        ...
        let renderActionItem = () => {
            if (this.state.activeTab == tabs.length - 1) {
                return (<ActionItem img={'/images/ic_add_white.png'} text={'새 에셋 만들기'}
                                    onClick={() => window.open("/editor/asset", "_blank")}/>);
            }
        }

        let renderContents = (assets) => {
            return (<AssetDetail id={this.state.selectedAssetIndex}/>)
            if (!!this.state.selectedAssetIndex) {
            } else {
                return (
                    <div>
    ...
    previewAsset(id) {
        this.setState({selectedAssetIndex: id});
    }
