...
function facebookLogin(){
  ...
  if (response.status === 'connected') {
    ...
    FB.api('/me', 'get', { access_token: accessToken, fields: 'id, name, email, picture' }, function(response) {
        $.ajax({
          type: 'POST',
          url: '/account/facebook',
          data: {"accessToken":accessToken, email : response.email, name: response.name, profile: response.picture.data.url},
          complete : function() {
            location.reload();
          }
        });
